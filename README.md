...
* `writeAssistAi.resultMode`: Controls what happens after a rewrite is generated. Use `acceptReject` to show inline Accept/Reject actions, `insertBelow` to insert the rewrite below the selected text, or `replace` to immediately replace the selected text. The default is `acceptReject`.
* `writeAssistAi.useAcceptRejectFlow`: Deprecated. Use `writeAssistAi.resultMode` instead. Existing configurations are migrated to `writeAssistAi.resultMode` when the extension activates.
* `writeAssistAi.separatorText`: This option allows you to define the separator text that surrounds output when `writeAssistAi.resultMode` is set to `insertBelow`. By default, it is set to '*' repeated 32 times. If you prefer to remove the separators, you can do so by setting this option to an empty string.
...
### AI response seems incomplete or cut short
