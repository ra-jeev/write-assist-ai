...
* `writeAssistAi.openAi.reasoningEffort`: Controls the amount of reasoning the model does before generating a response. Higher values may lead to more thoughtful responses but can increase latency and cost. This setting is primarily for reasoning models like `gpt-5`, `o1` etc. The default is `auto`.
